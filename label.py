...
# ─── 配置区 ──────────────────────────────────────────────────────────────────────
DATASET_SIZE        = 7473                       # 评测题目数量；None 表示整套
PRED_PATH           = "dataset/gsm8k_7473_llama.pt"    # torch.save 的 (ans_list, all_step_probs)
...
def main() -> None:
    ...
    ds_full = load_dataset("gsm8k", "main", split="train")
    ...
    with open(OUT_CSV, "w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["idx", "question", "reference_answer", "predicted_answer", "verdict"]
        )
